function LoadCommandPanel()
	...
	COMMAND_WINDOW.Paint = function( self, w, h )
		...
		draw.SimpleText( "COMMANDS", "RP_SubFontThick", 24, 20, Color( 190, 190, 190 ) )
	...
	table.insert( Categories, { Name = "MONEY", Table = MONEYCMD_BUTTONS } )
	table.insert( Categories, { Name = "ROLEPLAY", Table = RPCMD_BUTTONS } )
	if table.HasValue( ELITE.AccessToCPCmds, LocalPlayer():Team() ) then
		table.insert( Categories, { Name = "CIVIL PROTECTION", Table = CPCMD_BUTTONS } )
	end
	if table.HasValue( ELITE.AccessToMayorCmds, LocalPlayer():Team() ) then
		table.insert( Categories, { Name = "MAYOR", Table = MAYORCMD_BUTTONS } )
	end
	table.insert( Categories, { Name = "OTHER", Table = OTHERCMD_BUTTONS } )
